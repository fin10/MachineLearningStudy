...
def init_training_data(input):
    ...
    for domain in input:
        ...
        with open(domain['train'], 'r') as train_file:
            train = [[sentence.strip().lower(), domains.index(domain['domain'])] for sentence in train_file]
        with open(domain['test'], 'r') as test_file:
            test = [[sentence.strip().lower(), domains.index(domain['domain'])] for sentence in test_file]
...
def init_batch(data, batch_size, label_count):
    ...
    for i in range(batch_size):
        ...
        for word in data[i][0].split():
            if word in voca :    sentence.append(voca[word])
            else :               sentence.append(UNK)
